...
const StoreCommand = require('./selectStoreCmd');
...
class FloopybirdDAO {
  ...
  async AddPlayerVault(wallet_id) {
    ...
    await this.RunCommand(cStoreCommand.AddPlayerVaultCommand, {
      $wallet_id: wallet_id,
      $created_date: date,
    });
  }
  ...
  async WithdrawPlayerBalance(wallet_id, amount) {
    let date = parseInt(new Date().getTime() / 1000);
    let result = await this.RunCommand(cStoreCommand.WithdrawPlayerBalanceCommand, {
      $wallet_id: wallet_id,
      $amount: amount,
    });
    if (result == null || result.length == 0) return null;
